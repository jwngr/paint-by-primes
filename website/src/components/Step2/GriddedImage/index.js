...
const GriddedImage = ({src, imageWidth, pixelWidth, imageHeight, pixelHeight, scaleFactor}) => {
  ...
  for (let i = 1; i < imageWidth / scaledPixelWidth; i++) {
    pixelLines.push(
      <HorizontalLine left={i * scaledPixelWidth} height={imageHeight} key={`horizontal-line-${i}`}>
        &nbsp;
      </HorizontalLine>
    );
  }
